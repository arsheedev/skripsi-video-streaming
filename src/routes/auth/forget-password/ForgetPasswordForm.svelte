<script lang="ts">
	...
	let { data }: { data: { form: SuperValidated<Infer<typeof ForgetPasswordSchema>> } } = $props()
	...
	>
		<div class="mb-8 text-center">
			...
			<p class="text-sm text-gray-200/80">
				Please input your email, and we'll send you the link to reset your password
			</p>
		</div>
			...
			<Form.Field {form} name="email">
				<Form.Control>
					{#snippet children({ props })}
						<Form.Label class="mb-2 block text-sm font-medium text-gray-100">Email</Form.Label>
						<Input
							{...props}
							disabled={loading}
