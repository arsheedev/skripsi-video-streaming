<script lang="ts">
	...
	const alpha = Number(page.url.searchParams.get('n'))
		? 2 / (Number(page.url.searchParams.get('n')) + 1)
		: 0.1
	...
	function videoPlayer(node: HTMLVideoElement) {
		...
		const interval = setInterval(() => {
			...
			if (previousEma === 0) {
				ema = alpha * Number(downloadSpeed[count]) + (1 - alpha) * Number(downloadSpeed[count])
				previousEma = ema
			} else {
				ema = alpha * Number(downloadSpeed[count]) + (1 - alpha) * previousEma
				previousEma = ema
			}
